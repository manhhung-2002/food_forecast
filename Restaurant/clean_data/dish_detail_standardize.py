import argparse
from pyspark.sql import SparkSession
from clean_data.base_standardize import DataFrameStandardizer


class DishDetailStandardizer(DataFrameStandardizer):
    def _get_text_columns(self):
        # ingredient_name và unit là text
        return {
            "ingredient_name": "unknown",
            "unit": "unknown"
        }

    def _get_numeric_columns(self):
        # quantity là số, default = 0
        return {
            "quantity": 0
        }

    def _get_timestamp_columns(self):
        # không có timestamp
        return []

    def _get_accent_columns(self):
        # ingredient_name cần bỏ dấu
        return ["ingredient_name"]

    def _get_id_columns(self):
        # id, dish_id, ingredient_id giữ nguyên
        return ["id", "dish_id", "ingredient_id"]


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Standardize dish_detail table")
    parser.add_argument("--input", required=True, help="Path to input file")
    parser.add_argument("--output", required=True, help="Path to output file (single file)")
    parser.add_argument("--input_format", default="csv", help="Input format (csv/parquet)")
    parser.add_argument("--output_format", default="csv", help="Output format (csv/parquet)")
    args = parser.parse_args()

    spark = (
        SparkSession.builder
        .appName("DishDetailStandardize")
        .getOrCreate()
    )

    std = DishDetailStandardizer()
    std.standardize(
        spark,
        input_path=args.input,
        output_path=args.output,
        input_format=args.input_format,
        output_format=args.output_format
    )

    spark.stop()
